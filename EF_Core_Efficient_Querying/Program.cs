...
//                             .AsEnumerable()
//                             .Take(3)   //take in-memory de uygulanır.
//                             .ToList();

//önce where koşuluna göre hareket edilir sonra daha sonra in-memory de take fonksiyonu uygulanır.

//IQueryable da istene veriler içi sorgu yazılır ve excute edilerek çekilir yani ihtiyaca göre hareket sorgu esnasında sağlanır.hedef veri direkt alınır.
//aslnda IEnumerable a göre daha mantıklıdır.IEnumerable tüm veri alınır daha sonra ihtiyaçlar ayıklanır.
#endregion
...
//var persons = await context.Persons.Select(p => new   //select fokdiyonu ile yalnızca ihtiyaç olan kolonları listeliyoruz.
//{
//    p.Name
//}).ToListAsync();
#endregion
...
//var person = await context.Persons.Include(p => p.Orders).FirstOrDefaultAsync(p => p.PersonId == 1);  //oldukça maliyetlidir.

//var person = await context.Persons.FirstOrDefaultAsync(p => p.PersonId == 1); //idsi bir lan kişiyi aldım.

//if (person.Name == "Ayşe")  //ismi ayşe ise işlemlerimi yaptım.
//{
//    //Order'larını getir...
...
#endregion
#region İdeal Durum
//var persons = await context.Persons.Select(p => new { p.Name, p.Orders }).ToListAsync();   //orderlara urada gittik yani terar tekrar db ye gidilmesini engelledik.
