...
//await context.SaveChangesAsync();  //buraya kadar yazarsak veiiy kyıt etmez çünkü biz artıkkontrolü elimize aldık o yüzden commit denmelidir.

//await transaction.CommitAsync(); ///commt yazılmazsa default olarak rollback olarak devam eder..
#endregion
#region Savepoints
...
//await transaction.CreateSavepointAsync("t1");  //kayıt noktasını oluşturdum.

//Person p10 = await context.Persons.FindAsync(10);
//context.Persons.Remove(p10);
//await context.SaveChangesAsync();

//await transaction.RollbackToSavepointAsync("t1");  //t1 noktasına kadar olanları yap ondan sonrasını gei al demiş oldum.yani 10 id li kişi kalır 13 ve 11 idli kişi silinir.
...
//using TransactionScope transactionScope = new(); //hazır bir sınıftır bu sınıftan yeni bir obje türettik.
