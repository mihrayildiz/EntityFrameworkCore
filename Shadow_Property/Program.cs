...
//var blogs = await context.Blogs.Include(b => b.Posts)    //blogn postlarına ittik. shadow özelliği ile postların blogıd değerlerinide getirir.
//    .ToListAsync();
//Console.WriteLine();
#endregion
...
//        modelBuilder.Entity<Blog>()  blog entiysi içine DateTime tipinde CreatedDate lan bir shadow property oluşturdu.
...
//var blog = await context.Blogs.FirstAsync();  blogun içindei shadow property erimek istiyor b yüzden önce bloga erişmeliyi.

//var createDate = context.Entry(blog).Property("CreatedDate");  //blogun CreatedDate propertysii aldım
//Console.WriteLine(createDate.CurrentValue);  //şu an ki değeri
//Console.WriteLine(createDate.OriginalValue); //bir değişilki yaptım ama savechanges diyip dha dbye yollmada önceki değerlerini yazdım.
...
//blogların shadow propertysine göre orderby yaptık ve listeledik.
...
//post ile blog arasında bie çok ilşki var ve post classı içinde foreign key belirlenmemiş bu durumda ef core kendisi oluşturur
//yani shadow property olur.
class Post
{
    public int Id { get; set; }
