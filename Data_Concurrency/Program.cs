...
//önce veriyi select ile çekmeliyim. 
...
#endregion
#region WITH (XLOCK)
//using var transaction = await context.Database.BeginTransactionAsync();  //transaction elde edildi.
//var data = await context.Persons.FromSql($"SELECT * FROM Persons WITH (XLOCK) WHERE PersonID = 5")
//    .ToListAsync();
//Console.WriteLine();
//await transaction.CommitAsync();
//WITH (XLOCK)  ile seçtiğim veriye eriştiğimde veri bana gelee ladar hiç bir değişiklki yapma dedim. yani güncellemeye kapattım
#endregion
#endregion
...
public class Person
{
    ...
    [Timestamp]
    public byte[] RowVersion { get; set; }   //her bir satıra karşılık gelen versiyon bilgisni tutar ve Timestamp ile işaretlenmelidir.
}
class ApplicationDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        //modelBuilder.Entity<Person>().Property(p => p.Name).IsConcurrencyToken(); //Property Based Configuration (ConcurrencyCheck Attribute)
        modelBuilder.Entity<Person>().Property(p => p.RowVersion).IsRowVersion();  //RowVersion Column metodu
    }
