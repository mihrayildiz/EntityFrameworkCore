...
Person p = new()  //person classından p isimli bir nene türettin. ekleecek değelern verdim.
{
    Name = "Gençay",
    Surname = "Yıldız",
...
class Person
{
    //[DatabaseGenerated(DatabaseGeneratedOption.None)]  // PersonId kolonu identity özelliği kaldırıldı.
    //yani manuel data girebilisin.onun içide en son kalan değeri takip etmen lazım.böylece primary key ile identity
    //özelliği ayrılmış oldu ancak hala primary keydir.
    ...
    //[DatabaseGenerated(DatabaseGeneratedOption.Identity)] //PersonCode identity özellliği kondu yani db de ekleme yapıldığında kolon otomatik atacak.
    // public int  PersonCode { get; set; }

   // [DatabaseGenerated(DatabaseGeneratedOption.Identity)]
    //yine Identity özelliği verdk. ama sayısal olmayan bir türe verdik.aşağı NEWID verildi.
    //ıdetity olarak değil default olarak NEWID ye uygun değer üretir.
...
class ApplicationDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Person>()
            ...
            .ValueGeneratedNever(); //[DatabaseGenerated(DatabaseGeneratedOption.None)] ,PersonId koloud value generated etme dedi.

        modelBuilder.Entity<Person>()
            .Property(p => p.PersonCode)
            .HasDefaultValueSql("NEWID()"); //PersonCode kolonu NEWID ile unıque olarak üretisin dedi.

        modelBuilder.Entity<Person>()
            .Property(p => p.PersonCode)
            .ValueGeneratedOnAdd();  //identity olduğunu bildirdim.
    }
