...
class ApplicationDbContext : DbContext
{
    ...
    StreamWriter _log = new("logs.txt", append: true);  //logs.txt yi yoksa oluşturur varsa içne yazar.
    protected override async void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
        optionsBuilder.UseSqlServer("Server=localhost, 1433;Database=ApplicationDB;User ID=SA;Password=1q2w3e4r+!;TrustServerCertificate=True");

        //optionsBuilder.LogTo(Console.WriteLine); //console a hata yazıldı.
        //optionsBuilder.LogTo(message => Debug.WriteLine(message)); //debug penceresie loglama
        optionsBuilder.LogTo(async message => await _log.WriteLineAsync(message),LogLevel.Information)  //Information dedik hata bzında bilgi versin diye.
            .EnableSensitiveDataLogging() //hassas datalarıda getirir.
