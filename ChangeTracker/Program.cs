...
//var datas = context.ChangeTracker.Entries();  //Entries ifadesi ile daha save oladan önce gelecek olan güncel değeri göreiblrsin. dats üstüne gelirsen    
// udate edilen değerlerin yeni hallerini görebilirsin. 
...
//context.ChangeTracker.DetectChanges();  /save change arka tarafta ChangeTracker çalıştırsabile emin olmak içi yie kullanılabilir. detect eder 
//await context.SaveChangesAsync();  //ardından save il değişiklikler kayıt edilir.
...
//    if (e.State == EntityState.Unchanged)  //e nin durumları daha değişiklik olmadı alamında ise yani Unchanged 
//    {
//        //:..
//    }
...
//Console.WriteLine();  // yapılan değiiklik veritabanına kayıt edilmesdiyse değiişklik önces hali görülebilir.
#endregion

#region CurrentValues Property'si
//var urunAdi = context.Entry(urun).CurrentValues.GetValue<string>(nameof(Urun.UrunAdi));  // db de şu an ki değer getirir.
#endregion
