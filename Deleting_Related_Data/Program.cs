...
//if (person != null)  //person nullable yukarıda yani null gelebili ama null ifadeyi silemez hata almasın diye kontrol koyduk.
//    context.Addresses.Remove(person.Address); //ulaştığım id si 1 olan person un adresini sildim .
...
//    .Include(b => b.Posts)
//    .FirstOrDefaultAsync(b => b.Id == 1); //blog.Posts dolu gelir artık çünkü include ile bloglardn postları aldık..
...
//context.Authors.Remove(author); //Yazarı silmeye kalkar!!! yazar  tablosundanda siler.
////book.Authors.Remove(author);  //burada ise d si bir olan bookun ıdsi 2 olan yazar ile ilişkisini crosstableda siler.
//await context.SaveChangesAsync();
#endregion
...
context.Blogs.Remove(blog); //BLOG 2 yi silerken blog 2 ye bağlı olan diğer verilri de siler. post tablosunda log 2 ye bağlı deperleri de siler.
await context.SaveChangesAsync();
#endregion
...
{
    public int Id { get; set; }
    public int? BlogId { get; set; } // blog değerleri silindiği için primary key değerie gider o yüzden post taki BlogId nullable olması için ? koyduk.setnull operasyonu için
...
class ApplicationDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Post>()
           ...
           .WithMany(b => b.Posts)
           .IsRequired(false)
           .OnDelete(DeleteBehavior.SetNull);  //principa taloda bir deeğr sildiğinde eğer ki depent tabloda karşılığ varsa ilgili kolna null gelsin dedin.
        ...
        modelBuilder.Entity<Book>()  
            .HasMany(b => b.Authors)   //kitaplrdan yazarlara 
            .WithMany(a => a.Books); //yazarkrdan da kitaplara çoka çok ilişki
    }
}