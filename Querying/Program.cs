...
//              select urun;  // burada sadece sorgu var yani IQueryable

//urunId = 200;
//urunAdi = "4";

//foreach (Urun urun in urunler) //bu noktadan sonra artık execute oluyor ve enumerable olmş oluoyr.
//{
...
#endregion
#region Hiç Kayıt Gelmediğinde
//var urun = await context.Urunler.FirstAsync(u => u.Id == 5555); //5555 verisi yok bu durmda hata fırlatır.
#endregion
#region Çok Kayıt Geldiğinde
//var urun = await context.Urunler.FirstAsync(u => u.Id > 55);  //55ten büyük ik sayı 56 db bu yüzden 56 olan veriyi geiricek.
#endregion
#endregion
...
#region Tek Kayıt Geldiğinde
//var urun = await context.Urunler.FirstOrDefaultAsync(u => u.Id == 55); /ilk 55 değerini verir.
#endregion
#region Hiç Kayıt Gelmediğinde
//var urun = await context.Urunler.FirstOrDefaultAsync(u => u.Id == 5555);  //null değeri döndürür.
#endregion
#region Çok Kayıt Geldiğinde
...
//var urun = await context.Urunler.OrderBy(u => u.Fiyat).LastAsync(u => u.Id > 55);  //55ten büük olan en son ki ıd yi getirir.
#endregion
...
//var urunler = (await context.Urunler.ToListAsync()).Count();  //hepsini beleğe çeker performans yer maliyetlidir.
//var urunler = await context.Urunler.CountAsync(); // direk belleken int olrak sonucu döner.
#endregion
...
//var urunler = await context.Urunler.AnyAsync(u => u.UrunAdi.Contains("1"));  //yineIQueryable da çalışıyorz.
#endregion
...
//var m = await context.Urunler.AllAsync(u => u.Fiyat < 15000); //tüm ürünler 15000 den küçük bir fiyata sahip
//var m = await context.Urunler.AllAsync(u => u.UrunAdi.Contains("a")); // hpsinin içinde a geçiyor mu
#endregion
...
//var urunler = await context.Urunler.Select(u => new Urun  //burada Urun tipinde  istenilen kolonlar alınmştır.
//{
//    Id = u.Id,
//    Fiyat = u.Fiyat
//}).ToListAsync();
...
//aslında join yapısını kurmmızı sağlar. Arka tarafta inner join sorgusu çalışır.

//var urunler = await context.Urunler.Include(u => u.Parcalar).SelectMany(u => u.Parcalar, (u, p) => new   //her bir ürünün parçalarını getirdik.
//{
//    u.Id,
//    u.Fiyat,
//    p.ParcaAdi
